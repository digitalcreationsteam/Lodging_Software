...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    public function getContent()
    {
                    ...
                    if ($type_value['is_booking_product']) {
                        ...
                        foreach ($order_bk_data as $data_k => $data_v) {
                            ...
                            if (isset($cart_htl_data[$type_key]['date_diff'][$date_join])) {
                                ...
                                $cart_htl_data[$type_key]['date_diff'][$date_join]['data_form'] = date('d-m-Y h:i A', strtotime($data_v['date_from']));
                                $cart_htl_data[$type_key]['date_diff'][$date_join]['data_to'] = date('d-m-Y h:i A', strtotime($data_v['date_to']));
                        ...
                        Product::SELLING_PREFERENCE_HOTEL_STANDALONE == $type_value['selling_preference_type']
        ...
        $totalPaid = $footer['total_paid_real'];
        $f = new NumberFormatter("en", NumberFormatter::SPELLOUT);
        $amountInWords = ucfirst($f->format($totalPaid)) . ' only';
        $footer['amount_in_words'] = $amountInWords;
        ...
        $tpls = array(
            ...
            'description_tab' => $this->smarty->fetch($this->getTemplate('invoice.description-tab')),  // 
            'signature_tab' => $this->smarty->fetch($this->getTemplate('invoice.signature-tab')),      // 
